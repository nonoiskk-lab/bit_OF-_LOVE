  ...
  return (
    <section className="px-5 md:px-8 py-20 md:py-28 bg-white text-lb-charcoal">
      <div className="mx-auto max-w-6xl grid md:grid-cols-2 gap-10 md:gap-16 items-center">
        ...
        >
          <p className="font-number text-xs md:text-sm tracking-[0.3em] uppercase mb-3 text-lb-red">
            {number} — {eyebrow}
          </p>
          <h2 className="font-display font-bold uppercase text-4xl sm:text-5xl leading-[0.95] mb-5">
            {title}
          </h2>
          <p className="max-w-md mb-8 text-lb-neutral">{subtitle}</p>
          <Link
            href={href}
            className="inline-flex rounded-full bg-lb-red px-7 py-3.5 text-sm font-semibold text-lb-cream hover:bg-lb-red-deep transition-colors"
          >
            {cta}
          </Link>
        </motion.div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          {items.map((item, i) => (
            ...
            >
              ...
              <div className="p-3 bg-lb-cream-soft rounded-b-xl">
